...
| B27 | `supabase/migrations/0008_seed_admin.sql`                  | Admin password was hard-coded in a committed migration (FIXED — no default password is set now; operator must reset). | Change after first login; move secrets out of migrations.                  |
...
| #   | Item                                                                                                                                                          | Action                                                                                                                                                          |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S1  | `0008_seed_admin.sql` hard-coded the plaintext admin password (FIXED — the migration now sets a random, unusable placeholder and no credential is committed). | Rotate any password that was ever committed; create/reset admins only via the Supabase dashboard or an encrypted_password UPDATE with your own strong password. |
| S2  | `customer_name/email/phone` PII on `bookings`.                                                                                                                | Public RPCs return slot coordinates only; `get_paddle_status` now returns a FIRST NAME only (H4). Never expose the PII columns via anon RPCs.                   |
| S3  | No rate limiting / CAPTCHA on auth or contact forms.                                                                                                          | Enable Supabase Auth built-in rate limits; references are now non-enumerable (C3) so guest lookups cannot be brute-forced by reference.                         |
| S4  | Realtime broadcasts booking events to every client.                                                                                                           | Channel filtered per user AND Supabase Realtime enforces RLS on `bookings` — verify RLS is enabled for Realtime in the dashboard (H5).                          |
...
7. **B27/S1** rotate the previously committed admin password (FIXED in code — reset it in the Supabase dashboard).
